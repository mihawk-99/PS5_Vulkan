...
layout(set = 0, binding = 0) uniform samplerBuffer texels;
layout(location = 0) out vec4 color;

void main()
{
    const int index = int(gl_FragCoord.x) / 960;
    const vec4 constant = texelFetch(texels, 0);
    const vec4 quarter = texelFetch(texels, index);
    color = vec4(quarter.r, constant.r, 0.25, 1.0);
}
