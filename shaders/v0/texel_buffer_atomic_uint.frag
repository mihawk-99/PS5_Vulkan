#version 450
// PS5 Vulkan - R32_UINT atomics through a storage texel buffer.
// Copyright (C) 2026 Mihawk-99
// SPDX-License-Identifier: GPL-3.0-or-later
//
// The CTS requires VK_FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_ATOMIC_BIT for
// R32_UINT (dEQP-VK.api.info.format_properties.r32_uint), and this stage is the
...
layout(set = 0, binding = 0, r32ui) uniform uimageBuffer texels;
layout(location = 0) out vec4 color;

void main()
{
    const int index = int(gl_FragCoord.x) / 960;
    imageAtomicAdd(texels, index, 1u);
