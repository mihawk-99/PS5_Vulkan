...
"""Replay the r62-uniform-index capture through the host driver without tolerances."""
import os
from pathlib import Path
import subprocess
import tempfile
root = Path.cwd()
golden = root / 'golden/r62-uniform-index/run-1.json'
work = Path(tempfile.mkdtemp(prefix='r62-replay-', dir=root / 'build'))
case = 'r62-uniform-index'
directory = work / case
directory.mkdir()
...
subprocess.run(['python3', 'tools/golden.py', 'compare-run', str(golden), str(dump),
                '--test', case], check=True)
print('PASS: r62-uniform-index replays word for word')
