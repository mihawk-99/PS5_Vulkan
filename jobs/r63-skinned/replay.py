...
"""Replay the skinned-record capture through the host driver without tolerances."""
import os
from pathlib import Path
import subprocess
import tempfile
root = Path.cwd()
golden = root / 'golden/r63-skinned/run-1.json'
work = Path(tempfile.mkdtemp(prefix='r63-replay-', dir=root / 'build'))
case = 'r63-skinned'
directory = work / case
directory.mkdir()
...
subprocess.run(['python3', 'tools/golden.py', 'compare-run', str(golden), str(dump),
                '--test', case], check=True)
print('PASS: r63-skinned replays word for word')
