...
"""Replay the restart-split capture through the host driver without tolerances."""
import os
from pathlib import Path
import subprocess
import tempfile
root = Path.cwd()
golden = root / 'golden/r65-restart-split/run-1.json'
work = Path(tempfile.mkdtemp(prefix='r65-replay-', dir=root / 'build'))
case = 'r65-restart-split'
directory = work / case
directory.mkdir()
...
subprocess.run(['python3', 'tools/golden.py', 'compare-run', str(golden), str(dump),
                '--test', case], check=True)
print('PASS: r65-restart-split replays word for word')
