...
"""Replay the r59-fragcoord capture through the host driver without tolerances."""
import os
from pathlib import Path
import subprocess
import tempfile
root = Path.cwd()
golden = root / 'golden/r59-fragcoord/run-1.json'
work = Path(tempfile.mkdtemp(prefix='r59-replay-', dir=root / 'build'))
case = 'r59-fragcoord'
directory = work / case
directory.mkdir()
...
subprocess.run(['python3', 'tools/golden.py', 'compare-run', str(golden), str(dump),
                '--test', case], check=True)
print('PASS: r59-fragcoord replays word for word')
