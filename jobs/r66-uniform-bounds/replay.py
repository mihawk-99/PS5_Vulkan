...
"""Replay the uniform-bounds capture through the host driver without tolerances."""
import os
from pathlib import Path
import subprocess
import tempfile
root = Path.cwd()
golden = root / 'golden/r66-uniform-bounds/run-1.json'
work = Path(tempfile.mkdtemp(prefix='r66-replay-', dir=root / 'build'))
case = 'r66-uniform-bounds'
directory = work / case
directory.mkdir()
...
subprocess.run(['python3', 'tools/golden.py', 'compare-run', str(golden), str(dump),
                '--test', case], check=True)
print('PASS: r66-uniform-bounds replays word for word')
