...
"""Replay the primitive-restart capture through the host driver without tolerances."""
import os
from pathlib import Path
import subprocess
import tempfile
root = Path.cwd()
golden = root / 'golden/r58-restart/run-1.json'
work = Path(tempfile.mkdtemp(prefix='r58-replay-', dir=root / 'build'))
case = 'r58-restart'
directory = work / case
directory.mkdir()
...
subprocess.run(['python3', 'tools/golden.py', 'compare-run', str(golden), str(dump),
                '--test', case], check=True)
print('PASS: r58-restart replays word for word')
