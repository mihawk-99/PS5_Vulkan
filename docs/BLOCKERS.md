...
| 9 | ACO stability: the `aco::schedule_program` SIGFPE and the re-test of the depth-descriptor compile | **Closed: there was never an ACO fault.** The SIGFPE is the test runner dividing a sampled-format row's packed texel buffer by that row's zero texel size -- `std::array<SampledFormat, 10> sampled_unsigned_formats()` initialized seven rows, so `run_sampled_format_frames`'s eighth iteration divided `0 / 0` (`div %r13d`, `rax = rcx = rdx = 0`, `r13 = 0`; the same shape in `sampled_signed_formats`, declared 11 with seven rows). The ACO attributions of rounds 8, 15 and 16 came from symbolizing console addresses without their load base: every crash report in these runs carries `# /app0/eboot.bin` with `xotext: 0000000000400000:...`, the eboot is a PIE loaded at 0x400000, and rebased the three frames are `run_linked_agc_canary`, `main` and `_start`. Fixed in the runner (the tables declare their rows, `static_assert`s reject a declared-but-unfilled row, the cases take the table's own size) and proved on the console: `jobs/aco-min` pid 287, seven of seven tests PASS, each uint case reporting **"7 of 7 sampled formats fetched the colour their texel holds"**, `m2-solid` and `v0-formats` regressing, 1581 PASS records and no `signal:` record in the klog (`Klog_Logs/aco-min-tablefix.log`). The same case through the driver-enabled host runner no longer takes a signal either, and `get_addr_regs_from_waves`, the fork's `num_waves` guard and the stack-exhaustion reading are all retired with it (docs/HARDWARE_FINDINGS.md, 2026-09-20) |
...
- **ACO.** The fork's ACO differs from the pinned tree's in a handful of files
  ...
  0.3.0 fork and looking for the fault. **Measured: it is not.** The fork's guard
  is in `update_vgpr_sgpr_demand` (line 701) and is real, but the migration's
  witness on `get_addr_regs_from_waves` never fired in `jobs/aco-min` and the
  console fault reproduced unchanged (docs/HARDWARE_FINDINGS.md, 2026-09-20).
  The fault was not in ACO at all -- it was the runner's own zero divisor
  (row 9) -- so the fork's guard is a real guard for a fault this project has
  since found elsewhere, and the console's SIGFPE is closed without it.
