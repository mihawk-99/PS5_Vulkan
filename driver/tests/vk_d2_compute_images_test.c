...
int main(void)
{
   test_begin("D2 compute images");
#if defined(__linux__)
   size_t bytes = 0;
   const char *probes = getenv("PS5VK_PROBES");
   uint32_t *spirv = probes ? read_spirv(probes, "c0-images/dispatch.spv", &bytes) : NULL;
   ...
   if (spirv) {
      ...
      const struct ps5vk_compute_input input = {
         ...
         .entry_point = "main", .images = true, .report = {&steps, record_step},
      };
      ...
      if (ran) {
         ...
         if (count == 2) {
            ...
            for (uint32_t i = 0; i < 2; i++) {
               check(tables[i].address_low == (uint32_t)(uintptr_t)tables[i].words,
                     "the table pointer names the allocated table");
               check(tables[i].words[0] != 0 && (tables[i].words[2] & 0x3fffffffu) == (15u | (3u << 14)),
                     "image entry carries a GPU address and the 64x4 extent");
            }
            check(tables[0].words[8] != 0 && tables[1].words[8] == 0 &&
                     tables[1].words[9] == 0 && tables[1].words[10] == 0,
                  "the sampled entry carries a sampler and the storage entry stops at 32 bytes");
         }
#endif
      }
