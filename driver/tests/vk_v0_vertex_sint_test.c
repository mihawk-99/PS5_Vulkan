/*
 * PS5 Vulkan driver - V0-formats: the signed three-component integer vertex
 * position, through the Vulkan API.
 * Copyright (C) 2026 Mihawk-99
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 * Phase V0-formats (docs/V0_FORMATS_AUDIT.md). The audit lists
 * VK_FORMAT_R32G32B32_SINT as missing VERTEX_BUFFER alone, and the console's runner case
 * v0-vertex-sint drew the m3-vertex square with it: the integer position is
...
/* The m3-vertex square as signed integer records: position x, y and z at offset 0,
 ...
 * checks; x and y are ±1, which the set's shader scales by 0.5.
 */
...
main(void)
{
   test_begin("V0 vertex sint");
   size_t vertex_bytes = 0;
   size_t pixel_bytes = 0;
#if defined(__linux__)
   const char *const probes = getenv("PS5VK_PROBES");
   uint32_t *const vertex = probes ? read_spirv(probes, "v0-vertex-sint", "vertex", &vertex_bytes) : NULL;
   uint32_t *const pixel = probes ? read_spirv(probes, "v0-vertex-sint", "pixel", &pixel_bytes) : NULL;
   check(vertex && pixel, "PS5VK_PROBES holds the v0-vertex-sint SPIR-V");
#else
   ...
   if (vertex && pixel) {
      ...
      check(status == PS5VK_TRIANGLE_OK,
            "the indexed square with a signed integer position records, submits and signals "
            "its fence");
